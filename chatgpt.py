...
class GPT_Cal:
    ...
    def get_response(self):
        ...
        response = client.chat.completions.create(
            ...
            Message 我會給你任何格式的訊息，需要整理裡面的內容並對應上google calendar 的渲染方式，中文字需要編碼，如果開頭有"國會組報告"，那"預"字後的文字會有時間，"時"字後面會有內容，例如：國會組報告，國防部陸常次預5/9日(週四)，1400時於辦公室主持無人機預算說明。這代表"5/9(週四)，1400時"時間是20240509T1400，於辦公室代表地點在"立委辦公室"，後面的主持無人機預算說明代表內容為"陸常次主持預算說明會"
            Channel 將內容整理成標題、時間、地點、描述。範例: ['無人機預算說明', '202400509T140000/202400509T2330000', '委員辦公室', '具體描述']，並且要能整理出對應標題、行事曆時間、地點，其餘內容整理完後放在描述裡面，現在是 2024年，如果有兩筆資料中間會以分號；區隔，如果沒有分號(；)請依照語意分成另外一筆。
